		...
		case AMF_STRING:		// 字符串;
			{
				...
				printf("\t %s : \n", ByteToStr(valName.av_val, valName.av_len).c_str());
		...
		case AMF_ECMA_ARRAY:			// 数组;
			{
				...
				for (int i = 0; i < iArrLen; i++)
				{
					...
					case AMF_NUMBER:			// 数字(double);
						{
							printf("\t     %s : %.0f\n", ByteToStr(obj.o_props[i].p_name.av_val, obj.o_props[i].p_name.av_len).c_str(), obj.o_props[i].p_vu.p_number);
						}
						break;
					case AMF_STRING:			// 字符串; 
						{
							std::string strValue(obj.o_props[i].p_vu.p_aval.av_val, obj.o_props[i].p_vu.p_aval.av_len);
							printf("\t     %s : %s\n", ByteToStr(obj.o_props[i].p_name.av_val, obj.o_props[i].p_name.av_len).c_str(), strValue.c_str());
						}
						break;
					default:
						break;
